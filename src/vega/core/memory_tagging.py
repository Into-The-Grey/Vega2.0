...
class TaskMemory(Base):
    ...
    relevance_score = Column(
        Float, default=1.0
    )  # How useful was this knowledge (0.0-1.0)
    used_at = Column(
...
def _detect_task_heuristic(prompt: str) -> str:
    ...
    if scores:
        ...
        memory_logger.debug(
            f"Detected task type: {best_task} (score: {scores[best_task]})"
        )
        return best_task
...
def _detect_task_llm(prompt: str) -> str:
    ...
    # TODO: Implement LLM-based task detection
    # This would send a classification request to the LLM
    # For now, fall back to heuristic
    memory_logger.warning(
        "LLM-based task detection not yet implemented, using heuristic"
    )
    return _detect_task_heuristic(prompt)
    ...
    with Session(memory_engine) as session:
        for item_id in knowledge_item_ids:
            ...
            if existing:
                ...
                memory_logger.debug(
                    f"Updated task memory for item {item_id}, new score: {new_score:.2f}"
                )
            else:
                ...
                created_count += 1
                memory_logger.debug(
                    f"Created task memory for item {item_id}, task: {task_type}"
                )
...
def get_task_knowledge(
    ...
    with Session(memory_engine) as session:
        ...
        if include_general and task_type != TaskType.GENERAL_QUESTION:
            query = query.where(
                TaskMemory.task_type.in_([task_type, TaskType.GENERAL_QUESTION])
            )
        else:
        ...
        query = query.order_by(
            TaskMemory.relevance_score.desc(), TaskMemory.used_at.desc()
        )
    ...
    with Session(memory_engine) as session:
        ...
        if task_type:
            ...
            query = select(
                ...
                func.count(func.distinct(TaskMemory.knowledge_item_id)).label(
                    "unique_items"
                ),
            ...
            return {
                ...
                "success_rate": round(
                    (result.success_count or 0) / max(result.total_memories or 1, 1), 3
                ),
...
def search_by_tags(
    tags: List[str], task_type: Optional[str] = None, limit: int = 20
) -> List[KnowledgeItem]:
    """
    ...
    with Session(memory_engine) as session:
        ...
        memory_logger.info(
            f"Tag search for {tags} (task_type={task_type}): {len(results)} results"
        )
