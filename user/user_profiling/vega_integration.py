...
from user.user_profiling.engines.persona_engine import PersonaEngine, PersonaMode
from user.user_profiling.collectors.profile_intel_collector import ProfileIntelCollector
...
class UserProfileConfig(BaseModel):
    ...
    enable_contextual_responses: bool = Field(
        default=True, description="Enable contextual chat responses"
    )
    enable_auto_enrichment: bool = Field(
        default=True, description="Enable autonomous enrichment"
    )
    daemon_scan_interval_hours: int = Field(
        default=24, description="Full scan interval hours"
    )
    persona_update_interval_minutes: int = Field(
        default=30, description="Persona update interval"
    )
    max_context_injection_length: int = Field(
        default=500, description="Max context injection length"
    )
    privacy_mode: str = Field(
        default="balanced", description="Privacy mode: strict, balanced, open"
    )
...
class ContextualChatRequest(BaseModel):
    ...
    context_categories: List[str] = Field(
        default_factory=lambda: ["calendar", "academic", "personal"]
    )
...
class ContextualIntelligenceEngine:
    ...
    async def enhance_chat_response(
        ...
        try:
            ...
            persona_summary = self.persona_engine.get_persona_summary()
            current_mode = persona_summary.get("current_mode", PersonaMode.DEFAULT)

            # Get contextual information
            context_info = await self._gather_context_information(
            ...
            if config and getattr(config, "include_context", False) and context_info:
    ...
    async def _gather_context_information(
        ...
        try:
            ...
            if session_id:
                context["recent_interactions"] = await self._get_recent_interactions(
                    session_id
                )
    ...
    async def _get_calendar_context(self, session) -> Dict[str, Any]:
        ...
        from user.user_profiling.database.user_profile_schema import Calendar
        ...
        current_events = [
            ...
            if e.start_time.hour
            <= current_hour
            <= (e.end_time.hour if e.end_time else e.start_time.hour + 1)
        ]
        stress_level = max(
            [e.stress_level for e in current_events if e.stress_level], default=0.3
        )
        return {
    ...
    async def _get_academic_context(self, session) -> Dict[str, Any]:
        ...
        from user.user_profiling.database.user_profile_schema import EducationProfile
    ...
    async def _get_social_context(self, session) -> Dict[str, Any]:
        """Get social context"""
        try:
            from user.user_profiling.database.user_profile_schema import SocialCircle
                ...
                "relationship_types": {
                    "close_friends": len(
                        [
                            ...
                            if c.relationship_type == "friend"
                            and getattr(c, "closeness_level", 0) > 0.7
                        ]
                    ),
                    "family": len(
                        [c for c in connections if c.relationship_type == "family"]
                    ),
                    "professional": len(
                        [c for c in connections if c.relationship_type == "colleague"]
                    ),
                },
            }
        except ImportError:
    ...
    def _add_academic_urgency_awareness(
        self, response: str, academic_context: Dict[str, Any]
    ) -> str:
    ...
    async def _inject_contextual_awareness(
        self, response: str, context: Dict[str, Any], max_length: int
    ) -> str:
        ...
        if calendar_context.get("current_stress_level", 0) > 0.7:
            contextual_insights.append(
                "I notice you might be in a high-stress period based on your schedule."
            )

        if calendar_context.get("next_event"):
            next_event = calendar_context["next_event"]
            contextual_insights.append(
                f"Your next event is {next_event['title']} at {next_event['time']}."
            )
        ...
        if urgent_deadlines:
            deadline_text = (
                f"upcoming deadline{'s' if len(urgent_deadlines) > 1 else ''}"
            )
            contextual_insights.append(
                f"I see you have {len(urgent_deadlines)} {deadline_text} this week."
            )
    ...
    def _summarize_context(self, context: Dict[str, Any]) -> Dict[str, str]:
        ...
        if "calendar" in context:
            ...
            summary["calendar"] = (
                f"{cal.get('today_events_count', 0)} events today, stress level: {cal.get('current_stress_level', 0):.1f}"
            )

        if "academic" in context:
            acad = context["academic"]
            summary["academic"] = (
                f"{acad.get('active_courses', 0)} courses, {len(acad.get('urgent_deadlines', []))} urgent deadlines"
            )
...
class UserProfileManager:
    ...
    async def get_profile_summary(self) -> ProfileSummaryResponse:
        ...
        try:
            ...
            identity = {
                ...
                "location": (
                    identity_record.primary_location if identity_record else None
                ),
            }

            # Get current persona
            persona_summary = self.daemon.persona_engine.get_persona_summary()
            current_persona = persona_summary.get(
                "current_mode", PersonaMode.DEFAULT
            ).value
    ...
    async def _generate_current_recommendations(self) -> List[str]:
        ...
        try:
            ...
            briefing_recommendations = briefing.get("sections", {}).get(
                "recommendations", []
            )
    ...
    async def get_daily_briefing(self, date: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def update_profile_settings(self, settings: Dict[str, Any]) -> Dict[str, str]:
        ...
        try:
            ...
            return {"status": "success", "updated_settings": ",".join(settings.keys())}
...
async def shutdown_user_profiling():
    ...
    global user_profile_daemon
    if user_profile_daemon is not None:
...
def get_contextual_intelligence() -> ContextualIntelligenceEngine:
    ...
    if not user_profile_daemon:
        raise HTTPException(
            status_code=503, detail="User profiling system not available"
        )

    return ContextualIntelligenceEngine(
        user_profile_daemon.db, user_profile_daemon.persona_engine
    )
...
async def enhanced_chat_response(
    ...
    user_prompt: str,
    session_id: Optional[str] = None,
    intelligence_engine: ContextualIntelligenceEngine = Depends(
    ...
    try:
        enhanced_response, context_applied = (
            await intelligence_engine.enhance_chat_response(
        ...
        # Get understanding score
        understanding_score = (
            intelligence_engine.understanding_calculator.calculate_understanding_score()
        )
