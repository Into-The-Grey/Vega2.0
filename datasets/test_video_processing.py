...
def create_test_video(output_path: str, duration: int = 2, fps: int = 10, size: tuple = (320, 240)):
...
def test_get_video_info(video_processor, test_video):
    """Test comprehensive video information retrieval."""
    info = video_processor.get_video_info(test_video)
    
    assert 'validation' in info
    assert 'metadata' in info
    assert 'analysis_ready' in info
    
    assert info['validation']['is_valid']
    assert info['analysis_ready']
...
def test_convenience_functions(test_video):
    ...
    # Test video information function
    info = get_video_information(test_video)
    assert info['analysis_ready']
...
def test_video_converter_basic_functionality():
    """Test basic video converter functionality."""
    converter = VideoFormatConverter()
    
    # Test with non-existent file
    result = converter.copy_video_frames("nonexistent.mp4")
    assert result == ""