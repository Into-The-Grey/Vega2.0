...
class TestContentAnalyzer:
    ...
    @pytest.mark.asyncio
    async def test_content_analysis_basic(
        self, content_analyzer, sample_documents, processing_context
    ):
    ...
    async def test_content_analysis_empty_input(
        self, content_analyzer, processing_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_content_analysis_long_text(
        self, content_analyzer, processing_context
    ):
        ...
        if result.success:
            ...
            assert (
                "too long" in result.error.lower() or "exceed" in result.error.lower()
            )

    @pytest.mark.asyncio
    async def test_content_analysis_special_characters(
        self, content_analyzer, processing_context
    ):
...
class TestSemanticAnalyzer:
    ...
    @pytest.mark.asyncio
    async def test_semantic_analysis_basic(
        self, semantic_analyzer, sample_documents, processing_context
    ):
    ...
    async def test_topic_extraction(
        ...
        assert any(
            "ai" in text or "artificial" in text or "business" in text
    ...
    @pytest.mark.asyncio
    async def test_theme_identification(
        self, semantic_analyzer, sample_documents, processing_context
    ):
        ...
        if themes:
            ...
            assert any(
                "api" in name or "technical" in name or "documentation" in name
...
class TestSummaryGenerator:
    ...
    @pytest.mark.asyncio
    async def test_summary_generation_basic(
        self, summary_generator, sample_documents, processing_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_extractive_summary(
        self, summary_generator, sample_documents, processing_context
    ):
        """Test extractive summarization"""
        content = sample_documents["understanding_test"]
        input_data = {"text": content, "summary_type": "extractive", "max_sentences": 3}
    ...
    @pytest.mark.asyncio
    async def test_abstractive_summary(
        self, summary_generator, sample_documents, processing_context
    ):
        """Test abstractive summarization"""
        content = sample_documents["workflow_doc"]
        input_data = {"text": content, "summary_type": "abstractive", "max_length": 150}
    ...
    @pytest.mark.asyncio
    async def test_summary_different_lengths(
        self, summary_generator, sample_documents, processing_context
    ):
        ...
        for max_length in [50, 100, 200]:
            input_data = {"text": content, "max_length": max_length}
...
class TestEntityExtractor:
    ...
    async def test_entity_extraction_basic(
        self, entity_extractor, sample_documents, processing_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_organization_entity_extraction(
        self, entity_extractor, processing_context
    ):
        """Test organization entity extraction"""
        content = (
            "Apple Inc. and Microsoft Corporation are competing in the tech market."
        )
        result = await entity_extractor.process(content, processing_context)
        ...
        org_entities = [
            e for e in entities if e.get("label") in ["ORG", "ORGANIZATION"]
        ]
        if org_entities:
    ...
    async def test_custom_entity_patterns(self, entity_extractor, processing_context):
        ...
        content = (
            "Contact us at support@example.com or call 555-123-4567 for assistance."
        )
        result = await entity_extractor.process(content, processing_context)
        ...
        custom_entities = [
            e for e in entities if e.get("label") in ["EMAIL", "PHONE", "CONTACT"]
        ]
        # This may depend on custom pattern implementation

    @pytest.mark.asyncio
    async def test_entity_confidence_scoring(
        self, entity_extractor, processing_context
    ):
...
class TestDocumentUnderstandingAI:
    ...
    async def test_comprehensive_analysis(
        self, understanding_ai, sample_documents, processing_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_summary_generation_integration(
        self, understanding_ai, sample_documents, processing_context
    ):
    ...
    async def test_error_handling_cascading(self, processing_context):
        ...
        with patch.object(
            ai.content_analyzer, "process", side_effect=Exception("Component failed")
        ):
    ...
    async def test_timeout_handling(self):
        ...
        with pytest.raises((asyncio.TimeoutError, Exception)):
            await AsyncUtils.run_with_timeout(
                ai.analyze_content(large_content, context), timeout=1.0
            )
