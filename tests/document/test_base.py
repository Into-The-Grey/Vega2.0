    ...
    async def test_processing_timeout(self):
        ...
        with pytest.raises(asyncio.TimeoutError):
            await AsyncTestUtils.run_with_timeout(
                processor.process("test", context),
                timeout=1.0
            )
...
class TestAsyncUtilities:
    ...
    async def test_wait_for_condition_success(self):
        ...
        result = await AsyncTestUtils.wait_for_condition(condition, timeout=1.0, poll_interval=0.1)
        assert result is True
    ...
    async def test_wait_for_condition_timeout(self):
        ...
        result = await AsyncTestUtils.wait_for_condition(condition, timeout=0.2, poll_interval=0.1)
        assert result is False
