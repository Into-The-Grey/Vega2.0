...
class TestClassificationConfig:
    ...
    def test_custom_config(self):
        ...
        config = ClassificationConfig(
            min_confidence=0.9, max_categories=3, default_categories=custom_categories
        )
...
class TestClassificationCategory:
    ...
    def test_category_keyword_matching(self):
        ...
        category = ClassificationCategory(
            name="tech", keywords=["software", "programming", "code"]
        )
    ...
    def test_category_score_calculation(self):
        ...
        category = ClassificationCategory(
            name="business", keywords=["business", "company", "corporate", "enterprise"]
        )
...
class TestHierarchicalCategory:
    ...
    def test_hierarchical_creation(self):
        ...
        category = HierarchicalCategory(
            name="technology", level=1, parent=None, subcategories=[]
        )
...
class TestContentClassifier:
    ...
    @pytest.mark.asyncio
    async def test_content_classification_basic(
        self, content_classifier, sample_documents, processing_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_legal_document_classification(
        self, content_classifier, sample_documents, processing_context
    ):
        ...
        assert any(
            term in classification for term in ["legal", "contract", "agreement"]
        )

    @pytest.mark.asyncio
    async def test_technical_document_classification(
        self, content_classifier, sample_documents, processing_context
    ):
        ...
        assert any(
            term in classification for term in ["technical", "api", "documentation"]
        )

    @pytest.mark.asyncio
    async def test_academic_document_classification(
        self, content_classifier, sample_documents, processing_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_multi_category_classification(
        self, content_classifier, processing_context
    ):
        ...
        input_data = {"text": mixed_content, "multi_label": True, "max_categories": 3}
    ...
    async def test_confidence_scoring(self, content_classifier, processing_context):
        ...
        result_ambiguous = await content_classifier.process(
            ambiguous_content, processing_context
        )
...
class TestTopicClassifier:
    ...
    @pytest.mark.asyncio
    async def test_topic_classification_basic(
        self, topic_classifier, sample_documents, processing_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_ai_business_topic_identification(
        self, topic_classifier, sample_documents, processing_context
    ):
        ...
        assert any(
            "ai" in name or "artificial" in name or "business" in name
    ...
    async def test_topic_clustering(self, topic_classifier, processing_context):
        ...
        input_data = {"text": content, "cluster_topics": True, "max_topics": 5}
        ...
        if topics:
            ...
            assert any(
                "software" in name or "development" in name for name in topic_names
            )
...
class TestHierarchicalClassifier:
    ...
    @pytest.mark.asyncio
    async def test_hierarchical_classification_basic(
        self, hierarchical_classifier, sample_documents, processing_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_multi_level_classification(
        self, hierarchical_classifier, sample_documents, processing_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_legal_document_hierarchy(
        self, hierarchical_classifier, sample_documents, processing_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_classification_path_consistency(
        self, hierarchical_classifier, processing_context
    ):
    ...
    async def test_custom_hierarchy_definition(self, processing_context):
        ...
        if hierarchy:
            ...
            assert any(
                level in hierarchy.values()
                for level in ["business", "technology", "operations"]
            )
...
class TestIntentClassifier:
    ...
    @pytest.mark.asyncio
    async def test_intent_classification_basic(
        self, intent_classifier, processing_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_informational_intent(
        self, intent_classifier, sample_documents, processing_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_multiple_intent_detection(
        self, intent_classifier, processing_context
    ):
        ...
        input_data = {"text": mixed_content, "detect_multiple": True}
...
class TestDocumentClassificationAI:
    ...
    @pytest.mark.asyncio
    async def test_comprehensive_classification(
        self, classification_ai, sample_documents, processing_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_topic_classification_integration(
        self, classification_ai, sample_documents, processing_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_hierarchical_classification_integration(
        self, classification_ai, sample_documents, processing_context
    ):
        """Test hierarchical classification integration"""
        content = sample_documents["research_paper"]
        result = await classification_ai.classify_hierarchical(
    ...
    @pytest.mark.asyncio
    async def test_intent_classification_integration(
        self, classification_ai, processing_context
    ):
...
class TestPerformanceAndLoad:
    ...
    async def test_concurrent_classification(self):
        ...
        for i in range(5):
            content = (
                f"Document {i} about business technology and software development."
            )
    ...
    async def test_large_document_classification(self):
        ...
        """ + TestDataGenerator.generate_large_document(
            20
        )  # 20KB
        ...
        if not result.success:
            assert (
                "too large" in result.error.lower() or "memory" in result.error.lower()
            )
...
class TestErrorHandlingAndEdgeCases:
    ...
    async def test_component_failure_handling(self):
        ...
        with patch.object(
            ai.content_classifier, "process", side_effect=Exception("Component failed")
        ):
            context = TestFixtures.create_processing_context()
