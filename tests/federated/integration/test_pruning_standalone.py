...
sys.path.insert(0, str(ROOT_DIR / "src" / "vega" / "federated"))

from pruning import (  # type: ignore[import]
    PruningConfig,
    PruningType,
    SparsitySchedule,
    PruningCoordinator,
...
class TestModel(nn.Module):
    ...
    def __init__(
        self, input_size: int = 784, hidden_size: int = 512, num_classes: int = 10
    ):
        super().__init__()
...
async def test_basic_pruning() -> bool:
...
async def test_structured_pruning() -> bool:
    ...
    if metrics.model_size_reduction > 0.1:
...
async def test_knowledge_distillation() -> bool:
...
async def test_sparsity_aggregation() -> bool:
    ...
    print("✅ Sparsity-aware aggregation completed")
...
async def test_pruning_coordinator() -> bool:
    ...
    global_model = TestModel()
    participant_models: List[nn.Module] = [TestModel() for _ in range(3)]
...
async def test_sparsity_schedules() -> bool:
...
async def run_comprehensive_test() -> bool:
    ...
    test_functions: List[Tuple[str, Callable[[], Awaitable[bool]]]] = [
    ...
    for test_name, test_func in test_functions:
        try:
            ...
            print(f"❌ {test_name} failed with error: {exc}")
