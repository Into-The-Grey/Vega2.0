...
class TestSSLContext:
    ...
    def test_create_ssl_context_basic(self, mock_create_context):
    ...
    @patch("ssl.create_default_context")
    def test_create_ssl_context_with_ca(self, mock_create_context):
...
class TestAuditLogging:
    ...
    @patch("src.vega.federated.security.time.time")
    ...
    def test_audit_log_basic(self, mock_file, mock_time):
        ...
        # The function tries /var/log/vega/audit.log first
        mock_file.assert_called_with("/var/log/vega/audit.log", "a")
...
class TestAnomalyDetection:
    ...
    def test_detect_anomalies_normal_data(self):
        ...
        result = detect_anomalies(
            data, participant_id="participant_1", session_id="session_1"
        )
    ...
    def test_detect_anomalies_large_values(self):
        ...
        data = {
            "weights": [100.0, 2.0, 3.0],  # 100.0 exceeds threshold
        ...
        assert result["details"]["large_values"][0]["max_abs_value"] == 100.0
    ...
    def test_detect_anomalies_nan_values(self):
        ...
        assert result["is_anomalous"] is True
        assert "nan_or_inf_values" in result["anomaly_types"]
        assert "weights" in result["details"]["nan_or_inf_keys"]

    def test_detect_anomalies_inf_values(self):
        """Test anomaly detection with infinite values."""
        data = {
            "weights": [1.0, float("inf"), 3.0],
            "gradients": [0.5, -0.3, 0.8],
            "metadata": {"epoch": 5},
        }
...
class TestModelSignatures:
    ...
    def test_create_model_signature(self):
        ...
        model_bytes = json.dumps(model_data, sort_keys=True).encode()
        ...
        assert len(signature) == 32  # SHA-256 digest length (32 bytes)
    ...
    def test_verify_model_signature_tampered_data(self):
        ...
        original_bytes = json.dumps(original_data, sort_keys=True).encode()
        tampered_bytes = json.dumps(tampered_data, sort_keys=True).encode()
        secret_key = "test_secret_key"

        signature = create_model_signature(original_bytes, secret_key)
        result = verify_model_signature(tampered_bytes, signature, secret_key)
...
class TestParticipantDataValidation:
    ...
    def test_validate_participant_data_valid(self):
        ...
        result = validate_participant_data(data, "participant_1")
    ...
    def test_validate_participant_data_anomalous(self):
        ...
        data = {
            "participant_id": "participant_2",  # Wrong participant ID
            "timestamp": "2023-01-01T00:00:00Z",
            "weights": {
...
class TestAuditLoggerClass:
    ...
    def test_audit_logger_log(self, mock_file, mock_time):
        ...
        # AuditLogger.log() calls the global audit_log function, which tries /var/log/vega/audit.log first
        mock_file.assert_called_with("/var/log/vega/audit.log", "a")
        written_content = mock_file().write.call_args[0][0]
        log_entry = json.loads(written_content)
