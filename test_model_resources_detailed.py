            ...
            for file in ollama_dir.rglob("*"):
                if file.is_file():
                    ...
                    if size > 100_000_000:  # Files > 100MB
                        model_files.append({
                            "path": str(file.relative_to(ollama_dir)),
                            "size": size
                        })
            return {
                "total_size": total_size,
                "large_files": sorted(model_files, key=lambda x: x['size'], reverse=True)[:5]
            }
    except:
        pass
    return None
...
def main():
    ...
    try:
        result = subprocess.run(
            ['nvidia-smi', '--query-gpu=index,name,driver_version,memory.total,memory.used,memory.free,temperature.gpu,power.draw,power.limit',
             '--format=csv,noheader'],
    ...
    if ollama_running:
        ...
        for proc in ollama_procs:
            try:
                ...
                print(f"  PID {proc.info['pid']}: {mem_mb:.1f} MB RAM, {cpu_pct:.1f}% CPU")
            except:
                pass
    else:
            ...
            for i, line in enumerate(lines):
                ...
                if len(parts) == 2:
                    ...
                    if vram_free >= 6000:  # 6GB+ available
                        ...
                        elif "RTX" in gpu_name or "3060" in gpu_name or "3070" in gpu_name:
