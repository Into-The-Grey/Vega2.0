...
class VegaSmartAssistant:
    def is_tty_kvm(self) -> bool:
        """Detect if running in a TTY/KVM console (very basic check)"""
        ...
        if term in (
            ...
            return True
        # If SSH_TTY is not set and we're on /dev/tty* or /dev/console, likely KVM
        if not os.environ.get("SSH_TTY") and os.isatty(0):
            try:
                tty_name = os.ttyname(0)
                if tty_name.startswith("/dev/tty") or tty_name == "/dev/console":
                    return True
            except Exception:
                pass
        return False

    def tty_status_dashboard(self, report: dict):
        """Minimal TTY-friendly system status dashboard"""
        summary = report.get("summary", {})
        components = report.get("components", {})
        recommendations = report.get("recommendations", [])

        print("\n================ VEGA SYSTEM STATUS (TTY MODE) ================")
        print(f"Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(
            f"Components running: {summary.get('running_components', 0)}/{summary.get('total_components', 0)}"
        )
        if summary.get("stopped_components", 0):
            print(f"Stopped: {summary['stopped_components']}")
        if summary.get("error_components", 0):
            print(f"Errors: {summary['error_components']}")
        print("\nComponent Status:")
        for name, status in components.items():
            stat = status.get("status", "unknown")
            health = status.get("health", "n/a")
            print(f"- {name:20} {stat:10} health: {health}")
        if recommendations:
            print("\nRecommendations:")
    ...
    async def process_query(self, query: str) -> Dict[str, Any]:
        ...
        # Special handling for TTY/KVM system status
        try:
            if self.is_tty_kvm() and re.search(
                r"system status|status|check.*system|how.*doing", query, re.I
            ):
                # Use normal logic to create and execute the status task, but display with TTY dashboard
                ...
                # Extract report for TTY dashboard
                if (
                    result["success"]
                    and "data" in result
                    and "report" in result["data"]
                ):
                    self.tty_status_dashboard(result["data"]["report"])
                    return {
                        "success": True,
                        "response": "[TTY dashboard shown]",
                        "data": result["data"],
                        "task_id": task.id,
                    }
                else:
                    print("[TTY mode] Could not retrieve system status report.")
                    return result
